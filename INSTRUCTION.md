...
1. With the app running, go to `http://127.0.0.1:8000` in your browser.
...
7. As a quick backend health check, visit `http://127.0.0.1:8000/api/health` directly in your browser - you should see:
   ```
   {"status": "ok"}
   ```
...
Another program is using that port. Either close it, or open `.env` and change `PORT=8000` to `PORT=5050` (or any free number), then restart the app.
...
The app runs on your computer; you can access it from your phone's browser if both devices are on the same Wi-Fi network, by visiting `http://<your-computer's-local-IP>:8000` instead of `127.0.0.1`.
